                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <label for="">Nombre de page
                                ...
                                <input type="number" name="limit" value="{{ old('limit', 10) }}" id=""
                                    class="form-control" placeholder="Entrer">
                            </label>
                            ...
                            <label for="">Sélectionnez le mois
                                ...
                                <select name="month" id="" class="form-control">
                                    @foreach ($montharray as $key => $value)
                                        <option value="{{ $key }}" class="form-control">{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <tbody>
                                    ...
                                    <td style="color: black ">
                                        @if ($loop->first)
                                            <span @style(['color: #7e1615', 'font-weight: bold'])>
                                                {{ $costumer->orders_count }} Commandes / Montant total :
                                                {{ $costumer->orders_sum_total }}
                                    </td>

                                    </span>
                                @else
                                    {{ $costumer->orders_count }} Commandes / Montant total :
                                    {{ $costumer->orders_sum_total }} </td>
                            @endif
                            ...
                            <td style="color: black " class=" pull-right">
                                ...
                                {{--  <div class="btn-group btn-group-justified">
                                    <a href="{{ route('view-costumers', $costumer) }}" style="color: white" type="button"
